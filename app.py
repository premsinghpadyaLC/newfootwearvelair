        ...
        if order_submit:
            if not customer or not email:
                ...
                if qty > stock:
                    ...
                    pdf.cell(200, 10, "Footwear & Rice Store - Customer Invoice", ln=True, align='C', fill=True)
                    pdf.ln(8)

                    pdf.set_font("Arial", '', 12)
                    pdf.cell(100, 8, f"Invoice No: {invoice_num}", ln=True)
                    pdf.cell(100, 8, f"Date: {date}", ln=True)
                    pdf.cell(100, 8, f"Customer Name: {customer}", ln=True)
